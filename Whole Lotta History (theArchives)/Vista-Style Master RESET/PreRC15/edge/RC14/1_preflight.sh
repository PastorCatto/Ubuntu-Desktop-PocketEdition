...
if [ "$HOST_IS_ARM64" = "true" ]; then
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y \
        debootstrap sudo e2fsprogs curl wget make gcc \
        xz-utils gzip zip ca-certificates file git python3 \
        uuid-runtime android-sdk-libsparse-utils rsync dosfstools whiptail dialog lz4
else
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y \
        debootstrap qemu-user-static sudo e2fsprogs curl wget make gcc \
        xz-utils gzip zip ca-certificates file git python3 \
        uuid-runtime android-sdk-libsparse-utils \
        rsync dosfstools whiptail dialog lz4
fi
...
if [ "$UBUNTU_RELEASE" = "noble" ]; then
    ...
    echo "  Ensure firmware/${DEVICE_BRAND}-${DEVICE_CODENAME}/firmware.tar.gz"
    echo "  contains the UCM maps."
    echo "======================================================="
fi
...
echo "  11) custom"
echo ""
read -p "Choice [1-11]: " COLOR_CHOICE

case "$UBUNTU_RELEASE" in
    noble)    DEFAULT_COLOR="orange" ;;
    resolute) DEFAULT_COLOR="yellow" ;;
    *)        DEFAULT_COLOR="cyan"   ;;
esac
...
# -------------------------------------------------------
# Panel selection — only shown if device requires it
# -------------------------------------------------------
...
echo "2) Ubuntu Desktop     (ubuntu-desktop-minimal, GNOME)"
echo "3) Unity              (ubuntu-unity-desktop)"
echo "4) Plasma Desktop     (kde-plasma-desktop)"
echo "5) Plasma Mobile      (plasma-mobile)"
...
case $UI_CHOICE in
    ...
    6)
        ...
        echo "  Proceed only if you know what you are doing."
        echo "======================================================="
...
# Generated by 1_preflight.sh — RC14
...
BOOT_DTB_APPEND="${BOOT_DTB_APPEND:-false}"
BOOT_DTB="${BOOT_DTB:-}"
BOOT_PANEL_PICKER="${BOOT_PANEL_PICKER:-false}"
UBOOT_URL="${UBOOT_URL:-}"
UEFI_URL="${UEFI_URL:-}"
...
# -------------------------------------------------------
# Step 7: Watchdog / Auto-sudo options
# -------------------------------------------------------
echo "======================================================="
...
if [ "$AUTO_RUN" = "1" ]; then
    ...
    echo ">>> Auto-run complete. Run verify_build.sh then script 5."
else
    echo ">>> Stopped. Run scripts manually in order: 2 → 3 → verify_build.sh → 5"
fi
