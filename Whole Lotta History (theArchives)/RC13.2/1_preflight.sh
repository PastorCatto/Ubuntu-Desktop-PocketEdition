...
if [ "$HOST_IS_ARM64" = "true" ]; then
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y         debootstrap sudo e2fsprogs curl wget make gcc        xz-utils gzip zip ca-certificates file git python3         uuid-runtime android-sdk-libsparse-utils rsync dosfstools whiptail dialog
else
    # x86-64 host — QEMU required
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y         debootstrap qemu-user-binfmt-hwe sudo e2fsprogs curl wget make gcc         xz-utils gzip zip ca-certificates file git python3         uuid-runtime android-sdk-libsparse-utils         rsync dosfstools whiptail dialog
fi
