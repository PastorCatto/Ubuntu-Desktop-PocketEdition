...
source build.env
echo "======================================================="
echo "   Mobuntu — [3/5] RootFS Cooker -- RC10.3"
echo "======================================================="
...
if [ ! -d "$ROOTFS_DIR" ]; then
    ...
    # resolute-proposed uses resolute as the debootstrap suite
    DEBOOTSTRAP_SUITE="${UBUNTU_RELEASE}"
    [ "$UBUNTU_RELEASE" = "resolute-proposed" ] && DEBOOTSTRAP_SUITE="resolute"

    KEYRING_FLAG=""
    [ -f /usr/share/keyrings/ubuntu-archive-keyring.gpg ] && \
        KEYRING_FLAG="--keyring /usr/share/keyrings/ubuntu-archive-keyring.gpg"
    ...
    if [ "$HOST_IS_ARM64" = "true" ]; then
        ...
        sudo debootstrap $KEYRING_FLAG --arch=arm64 "$DEBOOTSTRAP_SUITE" "$ROOTFS_DIR" http://ports.ubuntu.com/
    else
        # x86-64 host — foreign debootstrap + QEMU second stage
        echo ">>> x86-64 host: running foreign debootstrap with QEMU..."
        sudo debootstrap $KEYRING_FLAG --arch=arm64 --foreign "$DEBOOTSTRAP_SUITE" "$ROOTFS_DIR" http://ports.ubuntu.com/
        sudo cp "${QEMU_BIN}" "$ROOTFS_DIR/usr/bin/"
        sudo chroot "$ROOTFS_DIR" "${QEMU_BIN##*/}" /bin/bash /debootstrap/debootstrap --second-stage
        if [ $? -ne 0 ]; then
...
# RC10.3: No pmaports device files
# RC10.3: Remoteproc udev rule removed — not reliable on SDM845
...
# RC10.2.1: No pmaports device files installed
...
# ------- Cleanup -------
rm -rf /tmp/kernel_payload /tmp/firmware.tar.gz /tmp/zz-qcom-bootimg /tmp/bootimg-initrd-hook /tmp/qcom-firmware /tmp/51-qcom.conf
apt-get clean
...
if [ "$HOST_IS_ARM64" = "true" ]; then
    ...
    sudo cp "${QEMU_BIN}" "$ROOTFS_DIR/usr/bin/"
    sudo chroot "$ROOTFS_DIR" "${QEMU_BIN##*/}" /bin/bash /tmp/chroot_setup.sh
fi
