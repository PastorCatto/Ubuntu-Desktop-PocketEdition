#!/bin/bash
# Mobuntu — RC13.4 (Snapshot Rollback)
set -e
source build.env
echo "======================================================="
echo "   Mobuntu — [3/5] RootFS Cooker -- RC13.4 (Snapshot Rollback)"
echo "======================================================="
...
if [ ! -d "$ROOTFS_DIR" ]; then
    ...
    # Use snapshot URL if set, otherwise rolling
    if [ -n "$UBUNTU_SNAPSHOT" ]; then
        DEBOOTSTRAP_MIRROR="http://snapshot.ubuntu.com/ubuntu-ports/${UBUNTU_SNAPSHOT}/"
        echo ">>> Debootstrap mirror: snapshot $UBUNTU_SNAPSHOT"
    else
    ...
    if [ "$HOST_IS_ARM64" = "true" ]; then
        ...
        sudo debootstrap \
            ...
            "$UBUNTU_RELEASE" "$ROOTFS_DIR" "$DEBOOTSTRAP_MIRROR"
    else
        ...
        sudo chroot "$ROOTFS_DIR" "$(basename $QEMU_BIN)" /bin/bash /debootstrap/debootstrap --second-stage
        if [ $? -ne 0 ]; then
...
for LOCAL_NAME in q6voiced.conf 81-libssc.rules; do
...
if [ "$FIRMWARE_METHOD" = "git" ] && [ -n "$FIRMWARE_REPO" ]; then
    ...
    # --- Priority 1: wholesale git clone ---
    ...
    if git clone --depth=1 "$FIRMWARE_REPO" "$FW_TMP/fw" 2>/dev/null; then
        ...
        # --- Fallback: local firmware archive ---
        if [ -f "$LOCAL_FW_ARCHIVE" ]; then
...
# RC13.4: Uses snapshot-4 (20260226T000000Z) if UBUNTU_SNAPSHOT is set.
# Snapshot-4 is the confirmed stable baseline for SDM845 on 26.04.
if [ -n "$UBUNTU_SNAPSHOT" ]; then
    echo ">>> Using Ubuntu snapshot: $UBUNTU_SNAPSHOT"
    printf "deb http://snapshot.ubuntu.com/ubuntu-ports/${UBUNTU_SNAPSHOT}/ ${UBUNTU_RELEASE} main restricted universe multiverse\n" > /etc/apt/sources.list
    printf "deb http://snapshot.ubuntu.com/ubuntu-ports/${UBUNTU_SNAPSHOT}/ ${UBUNTU_RELEASE}-updates main restricted universe multiverse\n" >> /etc/apt/sources.list
    printf "deb http://snapshot.ubuntu.com/ubuntu-ports/${UBUNTU_SNAPSHOT}/ ${UBUNTU_RELEASE}-security main restricted universe multiverse\n" >> /etc/apt/sources.list
else
...
# hexagonrpcd config (ADSP target — Mobian style)
echo ">>> Installing hexagonrpcd ADSP config..."
mkdir -p /etc/conf.d
cat > /etc/conf.d/hexagonrpcd << 'HEXEOF'
# Mobuntu -- RC13.1 hexagonrpcd config
# Target: ADSP (audio DSP on SDM845)
HEXAGONRPCD_ARGS="-d adsp -R /usr/share/qcom/sdm845/Xiaomi/beryllium"
HEXEOF
echo ">>>   hexagonrpcd ADSP config installed."
...
if [ "$HOST_IS_ARM64" = "true" ]; then
    ...
    sudo cp "$QEMU_BIN" "$ROOTFS_DIR/usr/bin/"
    sudo chroot "$ROOTFS_DIR" "$(basename $QEMU_BIN)" /bin/bash /tmp/chroot_setup.sh
fi
